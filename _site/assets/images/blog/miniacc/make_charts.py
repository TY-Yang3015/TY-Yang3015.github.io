...
def chart_individual_modules():
    ...
    ax.text(1.0, len(labels) - 0.25, "baseline", ha="center", va="bottom",
...
def chart_integrated():
    ...
    ]  # sorted ascending so fastest renders at top after inversion
    labels = [d[0] for d in data]
    values = [d[1] for d in data]
    ...
    ax.axvline(1.0, color=LABEL_GREY, linestyle="--", linewidth=0.9)
    ax.text(1.0, -0.55, "baseline", ha="center", va="top",
            fontsize=8, color=LABEL_GREY)
    ax.set_yticks(list(y), labels)
    ax.invert_yaxis()
