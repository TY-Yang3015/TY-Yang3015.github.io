MiniMax-H3 is a large diffusion model that turns a text prompt into a five-second video with synchronised audio. Serving it well from a single workstation GPU is awkward, because the model is far bigger than the card's memory. This post describes a small research programme around that problem. We measured where the time actually goes when you serve the model, tried a shelf of published acceleration ideas one by one, and then combined the ones that worked. The combination we now run finishes a request in 130 seconds where the baseline took 327 seconds on an RTX 4090, and we try to be honest about what that costs in output quality.
...
Kitchen INT8 & quantization & 1.881$\times$ & passed; single-run caveat\\
Compiler default & kernel & 0.970$\times$ & small changes\\
ViDiT-Q & quantization & --- & blocked before admission\\
...
The two clear winners attacked different costs. AdaLN removed per-layer conditioning work, and Kitchen halved the weight traffic. Combining them was the natural next step, and it is also where the story gets interesting. Figure~\ref{fig:breakdown} shows what happened to the anatomy of a request.
...
Figure~\ref{fig:integrated} shows the final speed numbers. AdaLN and Kitchen together give 2.19$\times$. Adding SageAttention on top reaches 2.51$\times$, our default configuration. Adding Sol instead gives 2.16$\times$.
...
There are several honest caveats. Speed numbers come from single requests on one GPU, so treat them as indicative rather than universal. The quality study is solid on its own scale, but 16 clips and 4 overall-consistency clips are still small samples, and the Sage cost would benefit from a bigger sweep before anyone ships this. We assessed video quality only. The audio track is validated for presence and format but never scored, because we know no good paired metric for generated audio. We used the original VBench rather than VBench-2 for a boring but important reason: we could pin and hash-verify every file of the original benchmark's scorer, which kept the whole evaluation reproducible. VBench-2 scores would not be comparable to ours anyway.
